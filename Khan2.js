...
function setupMenu() {
    ...
    function setupWatermark() {
        ...
        watermark.innerHTML = `<span style="text-shadow: -1px 0.5px 0 #72ff72, -2px 0px 0 #2f672e;">Zk</span> <span style="color:gray; padding-left:2px; font-family: Arial, sans-serif; font-size:10px">${ver}</span>`;
    ...
    function setupStatusPanel() {
        ...
        const update = async () => statsPanel.innerHTML = `
            <span style="text-shadow: -1px 0.5px 0 #p, -2px 0px 0 #2f672e;">Zk</span>
...
function setupMain(){
    ...
    function changeBannerText() {
        const phrases = [ "[🌿] Zacarias trabaindo.", "[🌿] Zacarias no controle.", "[🌿] Markes mamando", "[🌿] O melhor script.", "[🌿] Trabaindo", "[🌿] ZK" ];
        setInterval(() => { 
...
.then(()=>{
    ...
    /* Adicionar um player de música no menu */
function setupMusicPlayer() {
    const musicList = [
        { name: "Música 1", url: "https://store9.gofile.io/download/web/34d35067-dba8-458b-a736-3246c3b657de/Igual_Pc_Da_China_Speed_%5B_YouConvert.net_%5D.mp3" },
        { name: "Música 2", url: "https://example.com/musica2.mp3" },
        { name: "Música 3", url: "https://example.com/musica3.mp3" }
    ];
    ...
    // Adicionar o seletor de músicas no menu
    const musicLabel = document.createElement('label');
    musicLabel.textContent = "Selecione uma música:";
    musicLabel.style.cssText = "color: white; font-family: Arial, sans-serif; margin-top: 10px; display: block;";
    dropdownMenu.appendChild(musicLabel);
...
/* Chamada da função dentro do setupMenu */
setupMusicPlayer();
